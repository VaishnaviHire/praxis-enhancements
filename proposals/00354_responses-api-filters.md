...
origin:
  repo: ai
  issue: https://github.com/praxis-proxy/praxis/issues/354
  file: 00354_responses-api-filters.md
---
...
- `RequestExtensions` — type-safe, request-scoped extension container for heavy structured data (TypeMap pattern)
...
Filters communicate through three mechanisms:
...
#### Request Extensions (generic container)

A generic, type-safe, request-scoped extension container (`RequestExtensions`) on `HttpFilterContext`. Filters store and retrieve arbitrary typed values that persist across all Pingora lifecycle phases (request, request body, response, response body, logging).

The container follows the TypeMap pattern used by tower and axum for passing arbitrary state through request contexts. The framework has no knowledge of what filters store in it. The cost when unused is an empty `HashMap` (zero allocations, no overhead on existing filter chains).

**Design principles:**

- **Direct ownership, not `Arc<Mutex<>>`**: filters run sequentially within a request and do not need locking. Direct ownership avoids unnecessary synchronization overhead.
- **Coexists with `filter_metadata`**: lightweight routing keys (short strings, capped at 256 bytes per value) stay in metadata. Heavy request-scoped data (parsed bodies, conversation histories, streaming state) goes in extensions.
- **Cross-phase persistence**: extensions persist through error paths, matching the existing `filter_metadata` writeback pattern. This avoids repeating the cross-phase persistence bugs found in `executed_filter_indices` and `body_done_indices`.
- **Type isolation**: consumer types live in their own crates. Independent filters coexist in the same pipeline without collision risk.

**Motivation:** filters today have one channel for cross-phase communication: `filter_metadata`, a string-keyed map capped at 256 bytes per value. That is sufficient for routing keys and branch conditions, but not for structured, heavy request-scoped data. Without a generic extension container, each feature would either re-parse the request body in every filter that needs it, chunk structured data into 256-byte metadata values with ad-hoc serialization, or use side channels outside `HttpFilterContext`, bypassing the framework's lifecycle guarantees. All three are fragile. A generic extension container solves this once for all features.

The first consumer is `ResponsesState` (below). Future consumers include MCP session state, protocol negotiation state, and caching metadata. Each consumer defines its own types independently; the framework never needs to know about them.

**Non-goals for extensions:** persistent storage across requests (that is #412/#99), typed domain APIs for rate limits, sessions, or token ledgers (that is #99), migrating existing `filter_metadata` keys.

#### Orchestrator State (ResponsesState)

A request-scoped `ResponsesState` struct stored in `RequestExtensions`, created by `request_validate` during `on_request_body` and accessed by all downstream Responses filters. Holds the heavy data that would be too large for filter metadata (conversation history, tool definitions, streamed response state, etc.):
...
Filters access `ResponsesState` via the `RequestExtensions` container using type-safe lookup. Since extensions use direct ownership and filters run sequentially, no locking is required.
